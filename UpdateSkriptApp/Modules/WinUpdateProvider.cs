...
if ($updates.Count -gt 0) {{
    foreach ($u in $updates) {{
        $t = ([string]$u.Title).Replace('|', ' ').Replace('""', '').Replace("" `r"", """").Replace("" `n"", """").Trim()
        if ([string]::IsNullOrEmpty($t)) {{ $t = 'Unnamed Update' }}
        $k = ([string]($u.KB -join ', ')).Replace('|', ' ').Trim()
        Add-Content -Path '{tmpJsonFile}' -Value ""$t|$k"" -Encoding utf8
    }}
}} else {{
    # Create empty file
    New-Item -Path '{tmpJsonFile}' -ItemType File -Force | Out-Null
}}
";
        ...
        {
            throw new InvalidOperationException("Failed to get updates list. Output file not generated by PowerShell.");
        }

        var lines = _fs.ReadAllLines(tmpJsonFile);
        var pendingUpdates = new List<UpdateItem>();
        ...
        foreach (var line in lines)
        {
            ...
            if (parts.Length >= 2)
            {
                pendingUpdates.Add(new UpdateItem { Title = parts[0], KB = parts[1] });
            }
            else if (parts.Length == 1)
            {
                pendingUpdates.Add(new UpdateItem { Title = parts[0], KB = string.Empty });
            }
        }
        ...
        AnsiConsole.MarkupLine($"[yellow]Found {pendingUpdates.Count} pending updates.[/]");
